...
//#import "CZHHeader.h"
#define TitleFont Font(13)
#define ContentFont Font(20)
...
+ (instancetype)czh_itemWithTitle:(NSString *)title style:(CZHAlertItemStyle)style handler:(void (^)(CZHAlertItem *))handler {
    ...
    return [self czh_itemWithTitle:title titleFont:ContentFont titleColor:WhiteTextColor backgroundColor:MainDarkColor style:style handler:handler];
}
...
+ (instancetype)czh_itemWithTitle:(NSString *)title backgroundColor:(UIColor *)backgroundColor style:(CZHAlertItemStyle)style handler:(void (^)(CZHAlertItem *item))handler {
    return [self czh_itemWithTitle:title titleFont:ContentFont titleColor:WhiteTextColor backgroundColor:backgroundColor style:style handler:handler];
}
...
+ (instancetype)czh_itemWithTitle:(NSString *)title titleFont:(UIFont *)titleFont style:(CZHAlertItemStyle)style handler:(void (^)(CZHAlertItem *))handler {
    ...
    return [self czh_itemWithTitle:title titleFont:titleFont titleColor:WhiteTextColor backgroundColor:MainDarkColor style:style handler:handler];
...
+ (instancetype)czh_itemWithTitle:(NSString *)title titleColor:(UIColor *)titleColor style:(CZHAlertItemStyle)style handler:(void (^)(CZHAlertItem *))handler {
    ...
    return [self czh_itemWithTitle:title titleFont:ContentFont titleColor:titleColor backgroundColor:MainDarkColor style:style handler:handler];
...
+ (instancetype)czh_itemWithTitle:(NSString *)title titleFont:(UIFont *)titleFont titleColor:(UIColor *)titleColor style:(CZHAlertItemStyle)style handler:(void (^)(CZHAlertItem *))handler {
    ...
    return [self czh_itemWithTitle:title titleFont:titleFont titleColor:titleColor backgroundColor:MainDarkColor style:style handler:handler];
...
+ (instancetype)czh_itemWithTitle:(NSString *)title titleColor:(UIColor *)titleColor backgroundColor:(UIColor *)backgroundColor style:(CZHAlertItemStyle)style handler:(void (^)(CZHAlertItem *item))handler {
    return [self czh_itemWithTitle:title titleFont:ContentFont titleColor:titleColor backgroundColor:backgroundColor style:style handler:handler];
}
...
+ (instancetype)czh_itemWithTitle:(NSString *)title titleFont:(UIFont *)titleFont backgroundColor:(UIColor *)backgroundColor style:(CZHAlertItemStyle)style handler:(void (^)(CZHAlertItem *item))handler {
    return [self czh_itemWithTitle:title titleFont:titleFont titleColor:WhiteTextColor backgroundColor:backgroundColor style:style handler:handler];
}
...
/** 标题 */
@property (nonatomic, weak) BaseLabel *titleLabel;
/** 消息描述 */
...
+ (void)initialize {
    ...
    appearance.viewBackgroundColor = MainDarkColor;
    appearance.titleColor = TextTagGrayColor;
    appearance.messageColor = TextTagGrayColor;
    appearance.titleFont = TitleFont;
    appearance.messageFont = TitleFont;
...
- (void)czh_setUpView {
    ...
    wholeContainerView.backgroundColor = self.viewBackgroundColor;
...
//    NSAssert(self.title.length>0 || self.message.length >0 || self.itemArray.count > 0 || self.cancelItemArray.count > 0, @"please add a title or a message or a item");
    ...
    if (self.animationStyle == CZHAlertViewAnimationStyleDefault) {
        ...
        self.backgroundColor = [ColorFromHex(0x000000) colorWithAlphaComponent:0.3];
        ...
        self.wholeContainerView.easy_y = MAINSCREEN_HEIGHT;
        
        [UIView animateWithDuration:0.3 animations:^{
            self.backgroundColor = [ColorFromHex(0x000000) colorWithAlphaComponent:0.3];;
            
            if (self.preferredStyle == CZHAlertViewStyleActionSheet) {
                self.wholeContainerView.easy_y = self.easy_height - self.wholeContainerView.easy_height - (StatusBarH > 20 ? 10 : 0);
            } else if (self.preferredStyle == CZHAlertViewStyleAlert) {
                self.wholeContainerView.easy_y = (self.easy_height - self.wholeContainerView.easy_height)*0.5;
            }
        }];
    }
}
...
- (void)czh_hideView {
    ...
    if (self.animationStyle == CZHAlertViewAnimationStyleDefault) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            self.wholeContainerView.easy_y = self.easy_height;
...
- (void)czh_calculateFrameForAlert {
    ...
    CGFloat marginWidth = AdaptX(20);
    CGFloat marginHeight = AdaptX(15);
    
    CGFloat wholeContainerViewY = 0;
    CGFloat wholeContainerViewW = AdaptX(314);
    CGFloat wholeContainerViewX = (self.easy_width - wholeContainerViewW) * 0.5;
    ...
    if (_titleLabel.text.length > 0) {
        ...
        if (_messageLabel.text.length > 0) {
            buttonContainerViewH = AdaptX(29);
        } else {
    ...
    if (_messageLabel.text.length > 0) {
        ...
        buttonContainerViewH = CGRectGetMaxY(self.messageLabel.frame) + AdaptX(20);
    ...
    if (self.itemArray.count > 0) {
        ...
        firstLine.backgroundColor = ColorFromHex(0xdfdfdf).CGColor;
        ...
        CGFloat buttonHeight = AdaptX(50);
        ...
        for (NSInteger i = 0; i < itemCount; i++) {
            ...
            if (itemCount == 2) {
                if (i != itemCount - 1) {
                    ...
                    line.backgroundColor = MainBlackColor.CGColor;
    ...
    [_buttonContainerView cornerAllCornersWithCornerRadius:AdaptX(13)];
    
    wholeContainerViewY = (self.easy_height - wholeContainerViewH) * 0.5;
...
- (void)czh_calculateFrameForActionSheet {
    ...
    CGFloat wholeContainerViewY = 0;
    CGFloat wholeContainerViewW = AdaptX(355);
    CGFloat wholeContainerViewX = (self.easy_width - wholeContainerViewW) * 0.5;
    CGFloat wholeContainerViewH = 0;
    ...
    if (itemCount + cancelItemCount > 0) {
        ...
        CGFloat buttonHeight = AdaptX(60);
        CGFloat lineHeight = 0.5;

        for (NSInteger i = 0; i < itemCount; i++) {
            
            CALayer *firstLine = [CALayer layer];
            firstLine.backgroundColor = MainBlackColor.CGColor;
            [_buttonContainerView.layer addSublayer:firstLine];
        ...
        if (cancelItemCount > 0) {
            ...
            CGFloat buttonX = 0;
            CGFloat buttonY = buttonContainerViewH + AdaptX(10);
            CGFloat buttonW = buttonContainerViewW;
            CGFloat buttonH = buttonHeight;
            WeakSelf(self);
            CZHAlertButton *button = [CZHAlertButton czh_alertbuttonWithTitle:item.title titleColor:item.titleColor titleFont:item.titleFont backgroundColor:item.backgroundColor buttonHandler:^(CZHAlertButton *button) {
            ...
            [button cornerAllCornersWithCornerRadius:AdaptX(13)];
    ...
    wholeContainerViewY = self.easy_height - wholeContainerViewH;
    _wholeContainerView.frame = CGRectMake(wholeContainerViewX, wholeContainerViewY, wholeContainerViewW, wholeContainerViewH);
