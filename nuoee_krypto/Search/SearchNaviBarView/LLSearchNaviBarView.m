...
- (instancetype)init{
    if (self = [super init]) {
        ...
        self.frame = CGRectMake(0, 0, ZYHT_ScreenWidth, NavbarH);
...
- (UIButton *)rightOneBtn
{
    if (!_rightOneBtn) {
        ...
        _rightOneBtn = ({
            UIButton *rightOneBtn = [[UIButton alloc] initWithFrame:CGRectMake(ZYHT_ScreenWidth-leftRightIconWH, StatusBarH, leftRightIconWH, leftRightIconWH)];
...
- (UIView *)setupSearView
{
    ...
    self.searchBgView = ({
        UIView *searchBgView = [[UIView alloc] initWithFrame:CGRectMake(LeadMargin, StatusBarH+(NavbarH-StatusBarH-searchBgViewH)*0.5, ZYHT_ScreenWidth-TrailMargin-LeadMargin, searchBgViewH)];
        searchBgView.layer.cornerRadius =  2;//searchBgView.bounds.size.height*0.5;
        searchBgView.backgroundColor = ZYHT_Q_RGBAColor(46,46,46,1);
    ...
    self.searLogoIconV = ({
        ...
        searLogoIconV.image =  [UIImage imageNamed:@"search_nav"];
    ...
    self.searchBar = ({
        ...
        searchBar.leftImage = [UIImage imageNamed:@"search_left"];
        // searchNaviBarViewDefault 情况下参数
        searchBar.placeholderColor = TextDarkGrayColor;
...
- (void)updateFrameOnNav:(BOOL)onNav{

    if (onNav) {
        self.rightOneBtn.hidden = NO;
        self.frame = CGRectMake(0, 0, ZYHT_ScreenWidth, NavbarH);
        self.searchBgView.frame = CGRectMake(LeadMargin, StatusBarH+(NavbarH-StatusBarH-searchBgViewH)*0.5, ZYHT_ScreenWidth-TrailMargin-LeadMargin, searchBgViewH);
        [self modifySearchBarViewFrame];
    }else{
        self.rightOneBtn.hidden = YES;
        self.frame = CGRectMake(0, 0, ZYHT_ScreenWidth, AdaptY(52));
        self.searchBgView.frame = CGRectMake(LeadMargin, (self.frame.size.height-searchBgViewH)*0.5, ZYHT_ScreenWidth - 2 * LeadMargin, searchBgViewH);
        [self modifySearchBarViewFrame];
        [self.searchBar isHideClearButton];
        self.searchBar.text = @"";
    }
}
