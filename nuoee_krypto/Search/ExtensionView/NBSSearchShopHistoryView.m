...
@property (nonatomic, assign) NaviBarSearchType searchType;
...
{
    NBSSearchShopHistoryView *categoryView = [[NBSSearchShopHistoryView alloc] initWithFrame:rect seachType:searchType];
    categoryView.historyTagViewP = presenter;
    return categoryView;
}


-(instancetype)initWithFrame:(CGRect)frame seachType:(NaviBarSearchType)searchType
{
    if (self = [super initWithFrame:frame]) {
        ...
        if (self.searchType == NaviBarSearchTypeHistory) {
...
- (UILabel *)leftTitleLabel
{
    if (!_leftTitleLabel) {
        ...
        labelName.text = self.searchType == NaviBarSearchTypeHistory ? @"历史搜索" : @"热门搜索";
        labelName.font = ZYHT_F_MediumFontOfSize(14);
        labelName.textColor = TextDarkGrayColor;
...
- (UIButton *)rightMoreBtn
{
    if (!_rightMoreBtn) {
        ...
        [button setTitleColor:TextDarkGrayColor forState:UIControlStateNormal];
...
- (void)setuphistoryViewAndData
{
    ...
    [tagViewUtils setupCommonTagsInView:self.coverView tagTexts:self.searchType == NaviBarSearchTypeHistory ? [self.historyTagViewP getSearchCache] : self.hotArray margin:10 tagHeight:29];
    
    categoryViewHeight = self.coverView.height;
    tagViewUtils.commonTagStyle = ZYHTCommonTagStyleDarkTag;
