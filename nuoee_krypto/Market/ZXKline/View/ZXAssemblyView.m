...
- (void)updateConstrsinsForLandscape
{
    ...
    [self updateConstrainsWithWidth:self.klineMainViewWidth height:self.klineMainViewHeight];
    if (IsDisplayCandelInfoInTop) {
        self.topCandleInfoView.backgroundColor = [UIColor redColor];
        [self.topCandleInfoView mas_updateConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(self.mas_top).offset(TopMargin);
                if (PriceCoordinateIsInRight) {
                    
                     if (ZX_IS_IPHONE_X&&!Portrait) {
                         make.left.mas_equalTo(self).offset(SafeAreaBottomMargin);
                     }else{
    ...
    [currentDrawKlineModelArr enumerateObjectsUsingBlock:^(KlineModel *model, NSUInteger idx, BOOL * _Nonnull stop) {
        if (!model.isPlaceHolder) {
            ...
            if (model.openPrice<=model.closePrice) {
                [VOLColorArr addObject:[[SEUserDefaults shareInstance] getRiseOrFallColor:RoseType]];
            }else{
                [VOLColorArr addObject:[[SEUserDefaults shareInstance] getRiseOrFallColor:FallType]];
            }
        }
...
- (void)updateJumpViewWithNewKlineModel:(KlineModel *)newKlineModel
{
    ...
    if (newKlineModel.openPrice>newKlineModel.closePrice) {
        ...
        jumpViewBackgroundColor = [[SEUserDefaults shareInstance] getRiseOrFallColor:FallType];
    }else{
        jumpViewBackgroundColor = [[SEUserDefaults shareInstance] getRiseOrFallColor:RoseType];
    }
...
- (void)setIsDrawMA:(BOOL)isDrawMA{
    _isDrawMA = isDrawMA;
    self.klineMainView.isDrawMA = isDrawMA;
}
...
- (void)updateStopProfitLineViewWithPrice:(double)stopProfitPrice
{
    ...
    [self.stopProfitLineView updateJumpViewWithNewPrice:priceStr backgroundColor:[[SEUserDefaults shareInstance] getRiseOrFallColor:RoseType] typeText:@"TP"];
...
- (void)updateStopLossLineViewWithPrice:(double)stopLossPrice
{
    ...
    [self.stopLossLineView updateJumpViewWithNewPrice:priceStr backgroundColor:[[SEUserDefaults shareInstance] getRiseOrFallColor:FallType] typeText:@"SL"];
...
- (void)longpressPointCandleModel:(KlineModel *)klineModel longPressPoint:(CGPoint)point
{
    ...
    if (IsDisplayCandelInfoInTop) {
        ...
        if (klineModel.openPrice>klineModel.closePrice) {
            ...
            textColor = [[SEUserDefaults shareInstance] getRiseOrFallColor:FallType];
        }else{
            textColor = [[SEUserDefaults shareInstance] getRiseOrFallColor:RoseType];
        }
    ...
    if (maxPoint.x>SCREEN_WIDTH/2.0) {
        [self.maximumView mas_updateConstraints:^(MASConstraintMaker *make) {
            if (!Portrait&&ZX_IS_IPHONE_X) {
                ...
                make.bottom.mas_equalTo(self).offset(-maxPoint.y + SafeAreaBottomMargin);
            }else{
    ...
    if (minPoint.x>SCREEN_WIDTH/2.0) {
        [self.minimumView mas_updateConstraints:^(MASConstraintMaker *make) {
            ...
            if (!Portrait&&ZX_IS_IPHONE_X) {
                ...
                make.bottom.mas_equalTo(self).offset(-minPoint.y + SafeAreaBottomMargin);
            }else{
...
{
//    return CandleWidth - VerticalCoordinatesWidth;
    return CandleWidth;
}
...
- (ZXPriceView *)quotaView
{
    if (!_quotaView) {
        ...
        _quotaView.backgroundColor = [UIColor clearColor];
    }
    return _quotaView;
}
...
- (UIView *)priceMaskView
{
    if (!_priceMaskView) {
        ...
        _priceMaskView.backgroundColor = [UIColor clearColor];
