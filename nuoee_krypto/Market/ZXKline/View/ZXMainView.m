...
- (void)pinchAction:(UIPinchGestureRecognizer *)sender
{
    ...
    if (ABS(difValue)>Y_StockChartScaleBound) {
        ...
        if (self.kLineModelArr.count<self.needDrawKlineCount) {
            if (pinchOffsetY+self.subViewWidth>self.needDrawKlineCount*self.candleWidth) {
                
                pinchOffsetY = self.needDrawKlineCount*self.candleWidth - self.subViewWidth;
            }
        }else{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.kLineModelArr.count<self.needDrawKlineCount)
    {
        cell.model = self.needDrawKlineArr[indexPath.row];
    }else{
...
- (KlineModel *)calulatePositionWithKlineModel:(KlineModel *)kLineModel
{
    ...
    kLineModel.fillColor = kLineModel.openPrice <= kLineModel.closePrice ? [[SEUserDefaults shareInstance] getRiseOrFallColor:RoseType] : [[SEUserDefaults shareInstance] getRiseOrFallColor:FallType];
    kLineModel.strokeColor = kLineModel.openPrice <= kLineModel.closePrice ? [[SEUserDefaults shareInstance] getRiseOrFallColor:RoseType] : [[SEUserDefaults shareInstance] getRiseOrFallColor:FallType];
...
- (void)getCandyTimeWithPosition:(CGPoint)position
{
    ...
    if (self.kLineModelArr.count<self.needDrawKlineCount) {
        if (currentPositionIndexInDataArr>=self.needDrawKlineCount) {
    ...
    //    数据已经验证没问题
    KlineModel *moddel = nil;
    if (self.kLineModelArr.count<self.needDrawKlineCount) {
        moddel = self.needDrawKlineArr[currentPositionIndexInDataArr];
    }else{
        moddel = self.kLineModelArr[currentPositionIndexInDataArr];
    }
...
//    NSString *priceStr = [NSString stringWithFormat:@"%.*f",self.precision,[number doubleValue]];
    NSString *priceStr = [NSString stringWithFormat:@"%.2f",[number doubleValue]];
    return priceStr;
}
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        if (!candleWidth) {
            ...
            self.candleWidth = (CandleMaxWidth-CandleMinWidth)/2.0+CandleMinWidth;
            //将candle宽度保存到本地
            [[NSUserDefaults standardUserDefaults] setObject:@(self.candleWidth) forKey:kCandleWidth];
        }else
        {
